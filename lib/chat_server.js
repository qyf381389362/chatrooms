...
function joinRoom (socket, room) {
  ...
  socket.broadcast.to(room).emit('message', { // Socket.IO的broadcast函数是用来转发消息的
    text: nickNames[socket.id] + 'has joined' + room + '.'
  });
  let usersInRoom = io.sockets.clients(room); //确定有哪些用户在这个房间里
  if (usersInRoom.length > 1) {
    let usersInRoomSummary = 'Users currently in ' + room + ': '; // 如果不止一个用户在这个房间里，汇总下都是谁
    ...
    usersInRoomSummary += '.';
    socket.emit('message', {text: usersInRoomSummary}); // 将房间里其他用户的汇总发给这个用户
  }
}
